...
try:
    ...
    print("[INFO] spaCy not installed. Advanced parsing features disabled.")
...
def load_seed_symbols(file_path=SEED_PATH):
    ...
    if file_path_obj.exists() and file_path_obj.stat().st_size > 0:
        ...
        # No seed file found - start empty; seeds must be learned, not hardcoded
        return {}
...
def parse_text(text_input, current_lexicon=None):
    ...
    Routes to the appropriate parsing function and ensures required fields.
    """
    try:
        result = parse_raw_text(text_input, current_lexicon)
        
        # Ensure result has the required fields for system compatibility
        if isinstance(result, dict):
            # Add missing fields if not present
            if 'logic_score' not in result:
                # Simple heuristic: more keywords = more logical
                keywords = result.get('keywords', [])
                result['logic_score'] = min(8.0, len(keywords) * 1.5)
            
            if 'symbolic_score' not in result:
                # Simple heuristic: more symbols = more symbolic
                symbols = result.get('symbols', [])
                result['symbolic_score'] = min(8.0, len(symbols) * 2.0 + 1.0)
            
            # Ensure text field exists
            if 'text' not in result:
                result['text'] = str(text_input)
        
        return result
    except Exception as e:
        # Fallback: return minimal compatible result
        return {
            'text': str(text_input),
            'keywords': [],
            'symbols': [],
