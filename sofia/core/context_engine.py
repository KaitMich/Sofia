...
from sofia.utils.gpu_config import get_gpu_config

data_manager = DataManager()
gpu_config = get_gpu_config()

# Relationship tracker integration (lazy loading to avoid circular imports)
...
class ContextEngine:
    ...
    def __init__(self, model_name: str = 'all-MiniLM-L6-v2', data_dir: str = "data"):
        ...
        Args:
            ...
            data_dir: Data directory for relationship integration
        """
        self.model = SentenceTransformer(model_name, device=gpu_config.get_device_str())
        self.context_patterns = self._load_context_patterns()
        self.embedding_cache = {}
        self.learning_buffer = []
        self.ambiguous_terms = self._load_ambiguous_terms()
        
        # Initialize relationship tracker integration (lazy loading)
        self.relationship_tracker = None
        self._data_dir_for_relationships = data_dir
        self._relationships_load_attempted = False
    ...
    def learn_from_correction(self, text: str, context: str, 
        ...
        corrections = data_manager.get_json_data('context_corrections.json') or []
        corrections.append(learning_record)
        data_manager.save_json_data('context_corrections.json', corrections)
    ...
    def _load_ambiguous_terms(self) -> Set[str]:
        ...
        custom_terms = data_manager.get_json_data('ambiguous_terms.json') or []
        
        return default_terms.union(set(custom_terms))
    
    def _load_context_patterns(self) -> Dict:
        """Load learned context patterns"""
        patterns = data_manager.get_json_data('context_patterns.json') or {}
    ...
    def add_ambiguous_term(self, term: str, reason: Optional[str] = None):
        ...
        data_manager.save_json_data('ambiguous_terms.json', terms_list)
        ...
        if reason:
            ...
            logs = data_manager.get_json_data('term_additions.json') or []
            logs.append(log_entry)
            data_manager.save_json_data('term_additions.json', logs)
    ...
    def _get_relationship_tracker(self):
        """Lazy load the relationship tracker to avoid circular imports."""
        if not self._relationships_load_attempted:
            self._relationships_load_attempted = True
            try:
                from sofia.core.relationship_tracker import RelationshipTracker
                self.relationship_tracker = RelationshipTracker(self._data_dir_for_relationships)
            except Exception as e:
                print(f"⚠️ Relationship tracker integration failed: {e}")
                self.relationship_tracker = None
        return self.relationship_tracker
    
    def analyze_context_with_relationship_awareness(self, text: str, human_identifier: str,
                                                  surrounding_content: Optional[List[str]] = None,
                                                  conversation_history: Optional[List[Dict]] = None) -> Dict:
        """
        Enhanced context analysis that considers relationship history and dynamics.
        
        Args:
            text: The text to analyze
            human_identifier: Identifier for the human in the conversation
            surrounding_content: Additional context
            conversation_history: Previous conversation turns
            
        Returns:
            Enhanced context analysis with relationship insights
        """
        # Get basic context analysis first
        basic_analysis = self.analyze_context(text, surrounding_content, conversation_history)
        
        # Try to enhance with relationship insights
        relationship_tracker = self._get_relationship_tracker()
        if relationship_tracker:
            try:
                # Get relationship context
                relationship_context = relationship_tracker.get_relationship_context(human_identifier)
                
                # Enhance analysis with relationship awareness
                enhanced_analysis = self._enhance_with_relationship_context(
                    basic_analysis, relationship_context, text
                )
                
                # Provide feedback to relationship tracker about context insights
                self._provide_context_insights_to_relationship_tracker(
                    human_identifier, text, enhanced_analysis
                )
                
                return enhanced_analysis
                
            except Exception as e:
                print(f"⚠️ Relationship enhancement failed: {e}")
                return basic_analysis
        
        return basic_analysis
    
    def _enhance_with_relationship_context(self, basic_analysis: Dict, 
                                         relationship_context: Dict, text: str) -> Dict:
        """Enhance context analysis using relationship history."""
        enhanced = basic_analysis.copy()
        
        # Add relationship-aware confidence adjustment
        if relationship_context:
            # Get relationship depth and communication patterns
            relationship_depth = relationship_context.get("relationship_depth", 0.0)
            communication_patterns = relationship_context.get("communication_patterns", {})
            
            # Adjust confidence based on relationship familiarity
            familiarity_boost = min(0.2, relationship_depth * 0.3)
            enhanced["confidence"] = min(1.0, enhanced.get("confidence", 0.5) + familiarity_boost)
            
            # Add relationship-specific context clues
            enhanced["relationship_insights"] = {
                "relationship_depth": relationship_depth,
                "communication_style": communication_patterns.get("primary_style", "unknown"),
                "context_familiarity": familiarity_boost,
                "relationship_stage": self._determine_relationship_stage(relationship_depth)
            }
            
            # Adjust intent based on relationship patterns
            if communication_patterns:
                enhanced = self._adjust_intent_for_relationship_patterns(
                    enhanced, communication_patterns, text
                )
        
        return enhanced
    
    def _provide_context_insights_to_relationship_tracker(self, human_identifier: str, 
                                                        text: str, analysis: Dict):
        """Provide context insights back to relationship tracker for learning."""
        relationship_tracker = self._get_relationship_tracker()
        if relationship_tracker:
            try:
                # Create context insights for relationship tracker
                context_insights = {
                    ...
                    "communication_style": self._infer_communication_style(text, analysis),
                    "context_complexity": len(analysis.get("evidence", []))
                }
                
                # This would be called by relationship tracker if it had a method for receiving context insights
                # For now, we'll just prepare the insights
                setattr(self, '_latest_context_insights', {
                ...
                print(f"⚠️ Failed to provide context insights: {e}")
    
    def get_relationship_context_insights(self, human_identifier: str) -> Optional[Dict]:
        """Get the latest context insights for a specific human."""
        latest = getattr(self, '_latest_context_insights', None)
        if latest and latest.get('human_identifier') == human_identifier:
            return latest.get('insights')
        return None
    
    def _determine_relationship_stage(self, depth: float) -> str:
        """Determine relationship stage based on depth."""
        if depth < 0.2:
            return "initial"
        elif depth < 0.5:
            return "developing"
    ...
    def _adjust_intent_for_relationship_patterns(self, analysis: Dict, 
                                               patterns: Dict, text: str) -> Dict:
        """Adjust intent interpretation based on known relationship patterns."""
        # If we know this person uses humor frequently, adjust for that
        if patterns.get("humor_frequency", 0) > 0.7:
            if analysis.get("intent") in ["potentially_offensive", "unclear"]:
                analysis["intent"] = "humor_attempt"
                analysis["confidence"] = min(1.0, analysis.get("confidence", 0.5) + 0.2)
                analysis["evidence"] = analysis.get("evidence", []) + ["known_humor_pattern"]
        
        # If person typically uses academic language, adjust for that
        if patterns.get("academic_style", 0) > 0.6:
            if analysis.get("intent") in ["unclear", "casual"]:
                analysis["intent"] = "academic_discussion"
                analysis["confidence"] = min(1.0, analysis.get("confidence", 0.5) + 0.15)
        
        return analysis
    
    def _extract_emotional_context(self, analysis: Dict) -> str:
        """Extract emotional context from analysis."""
        intent = analysis.get("intent", "")
        if "positive" in intent or "celebration" in intent:
            return "positive"
        elif "negative" in intent or "hate" in intent:
            return "negative"
        elif "academic" in intent or "neutral" in intent:
            return "neutral"
        else:
            return "mixed"
    
    def _infer_communication_style(self, text: str, analysis: Dict) -> str:
        """Infer communication style from text and analysis."""
        text_lower = text.lower()
        ...
        # Check for technical/academic
        if analysis.get("intent") == "academic_neutral":
            return "academic"
...
# Singleton instance
context_engine = ContextEngine(data_dir="data")