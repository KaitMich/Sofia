...
# Import dependencies
from sofia.utils.vector_engine import fuse_vectors, embed_text
from sklearn.metrics.pairwise import cosine_similarity
from sofia.security.quarantine_layer import UserMemoryQuarantine, should_quarantine_input
...
class TripartiteMemory:
    ...
    def store(self, item, decision_type):
        """Store an item in bridge memory (universal intake).

        BRIDGE-FIRST ARCHITECTURE (March 27, 2026):
        All new content enters bridge regardless of decision_type.
        The classifier's decision is stored as metadata (initial_impression),
        ...
        with self.lock:
            ...
            if item_text is None or (isinstance(item_text, str) and len(item_text.strip()) < 20):
                return  # Silently reject garbage — no NoneType risk downstream

            # Store classifier suggestion as metadata, not routing
            if decision_type != 'FOLLOW_HYBRID' and 'initial_impression' not in item:
                item['initial_impression'] = {
                    'classifier_suggestion': decision_type,
                    'timestamp': datetime.utcnow().isoformat()
                }

            # Add metadata — everything goes to bridge
            item['stored_at'] = datetime.utcnow().isoformat()
            item['decision_type'] = 'FOLLOW_HYBRID'  # Bridge-first: always bridge

            # Compute and cache embedding for migration engine
            try:
    ...
    def _save_safe(self, filename, data):
        """Save with atomic write, backup, and retry on Windows file locks."""
        ...
        for attempt in range(3):
            try:
                ...
                # Windows file lock from another process (e.g., dashboard reading).
                # Brief retry — the reader releases within milliseconds.
                if attempt < 2:
                    _time.sleep(0.5)
                    continue
                print(f"❌ PermissionError saving {filename} after 3 attempts")
                if temp.exists():
                    temp.unlink()
                return False
    ...
    def remove_duplicates(self):
        ...
        with self.lock:
            # Remove duplicates from each memory type
...
class UnifiedMemory:
    ...
    def move_item_from_bridge(self, item: Dict, target_memory: str, reason: str) -> bool:
        """
        Move an item from bridge memory to logic or symbolic memory.
        Preserves full history for audit trail.

        Args:
            item: The bridge memory item to move
            target_memory: 'LOGIC' or 'SYMBOLIC'
        ...
        """
        try:
            acquired = self.tripartite.lock.acquire(timeout=5.0)
            if not acquired:
                print(f"⚠️ Could not acquire lock for moving item from bridge")
                return False

            try:
                # Verify item exists in bridge
                item_id = item.get('id')
                if not any(i.get('id') == item_id for i in self.tripartite.bridge_memory):
                    print(f"⚠️ Item {item_id} not found in bridge memory")
                    return False

                # Add reclassification metadata
                ...
                item['previous_decision'] = item.get('decision_type', 'FOLLOW_HYBRID')

                # Remove from bridge
                self.tripartite.bridge_memory = [i for i in self.tripartite.bridge_memory if i.get('id') != item_id]

                # Add to target memory
                if target_memory.upper() == 'LOGIC':
                    item['decision_type'] = 'FOLLOW_LOGIC'
                    self.tripartite.logic_memory.append(item)
                elif target_memory.upper() == 'SYMBOLIC':
                    item['decision_type'] = 'FOLLOW_SYMBOLIC'
                    self.tripartite.symbolic_memory.append(item)
                else:
                    print(f"❌ Invalid target memory: {target_memory}")
                    return False

                # Persist changes
                self.tripartite._save_tripartite_memory()

                print(f"✅ Reclassified {item_id} from bridge to {target_memory}: {reason}")
                return True

            finally:
                self.tripartite.lock.release()

        except Exception as e:
            print(f"❌ Failed to move item from bridge: {e}")
            return False

    def get_bridge_items_for_review(self, min_age_days: int = 7) -> List[Dict]:
        """
        Get bridge items eligible for reclassification review.

        Args:
            min_age_days: Minimum age in days for eligibility

        Returns:
            List of eligible bridge items
        """
        from datetime import datetime, timedelta
        ...
        for item in self.tripartite.bridge_memory:
            ...
            if timestamp:
                try:
                    ...
                    # If we can't parse date, include it (assume old enough)
                    eligible.append(item)
            else:
