...
class SmartLinkProcessor:
        ...
        try:
            ...
            for pattern in suspicious_patterns:
                try:
                    ...
                    print(f"[REGEX_ERROR] Invalid pattern '{pattern}': {e}")
                    continue
    ...
    def _discover_related_links(self, base_url: str, html_content: str, 
        ...
        for link_url, anchor_text in all_links:
            ...
            similarity_score = self._calculate_content_similarity(main_text, anchor_text, link_url, base_url)
    ...
    def _calculate_content_similarity(self, main_text: str, anchor_text: str, link_url: str, base_url: str) -> float:
        """
        ...
        # Use regex to properly match wikipedia.org domains
        try:
            if re.search(r'(^|\.)wikipedia\.org$', main_domain) or re.search(r'\.edu$', main_domain):
                similarity_score += 0.2
        except re.error as e:
            print(f"[REGEX_ERROR] Domain pattern matching failed: {e}")
            # Fallback to simple string matching
            if 'wikipedia.org' in main_domain or main_domain.endswith('.edu'):
                similarity_score += 0.2
        
        # Topic continuity (check if same base domain as main URL)
    ...
    def _extract_content_insights(self, text_content: str, url: str) -> Dict:
        ...
        for pattern in concept_patterns:
            try:
                ...
                print(f"[REGEX_ERROR] Concept pattern '{pattern}' failed: {e}")
                # Fallback to simple string matching
                if pattern.replace(r'\b', '').replace('\\', '') in text_lower:
                    insights['concepts_mentioned'].append(pattern.replace(r'\b', '').replace('\\', ''))
