...
class MigrationEngine:
    ...
    def should_migrate(self, item, new_decision, new_score, threshold):
        ...
        1. Sovereignty approval (identity protection)
        2. Score exceeds threshold
        3. Decision history shows stability
        4. No recent ping-ponging
        """
        # SOVEREIGNTY CHECK: Always check identity protection first
        if SOVEREIGNTY_AVAILABLE:
            # Check for protected content
            if is_protected_content(item):
                return False, "Protected by identity core - cannot migrate"
            ...
            migration_action = {
                ...
                "description": f"Migrate item to {new_decision}",
                "new_decision": new_decision,
                "score": new_score
            }
            
            sovereignty_result = sovereignty_check(migration_action)
            if sovereignty_result["veto"]:
                return False, f"Sovereignty veto: {sovereignty_result['reasoning']}"
            elif not sovereignty_result["approved"] and sovereignty_result["conflicts"]:
                return False, f"Sovereignty conflict: {sovereignty_result['conflicts'][0]['description']}"
