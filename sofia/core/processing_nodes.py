...
from sofia.memory.unified_weight_system import UnifiedWeightSystem
...
from sofia.core.unified_memory import UnifiedMemory, get_unified_memory

from sofia.utils import parser as P_Parser
from sofia.utils import emotion_handler as EH_EmotionHandler
# Symbol emotion updating is now part of unified symbol system
from sofia.core.unified_symbol_system import UnifiedSymbolSystem, get_unified_symbol_system
...
def evaluate_link_with_confidence_gates(logic_score, symbolic_score, logic_scale=2.0, sym_scale=1.0):
    ...
    """
    from sofia.memory.adaptive_migration import evaluate_link_with_confidence_gates as _canonical
...
def detect_content_type(text_input: str, spacy_nlp_instance=None) -> str:
    """Grounded content-type detection (factual / symbolic / ambiguous).

    Delegates to content_classifier.GroundedClassifier: emergent cluster
    centroids when Sofia's stores are mature, measured bootstrap signals
    (emotion model, structural density) before that. Replaces ~60 hardcoded
    topic/marker keywords. spacy_nlp_instance is accepted for call-site
    ...
    try:
        ...
        label = get_classifier().classify(text_input).label
    except Exception:
        return "ambiguous"
    return {"logical": "factual", "symbolic": "symbolic"}.get(label, "ambiguous")
...
class CurriculumManager: # Incorporating Step 1 changes
    def __init__(self):
        ...
        # Phase keywords for relevance scoring. Anti-keywords REMOVED (March 28, 2026):
        # Guidelines tell Sofia where she CAN start. Code never tells her where she CANNOT go.
        # See data/seed_coordinates_manifest.json for structural scaffolding.
        self.phase_data_sources_keywords = {
             1: {"primary": ["algorithm", "data structure", "computational complexity", "software architecture", "turing machine", "von neumann", "boolean algebra"],
                 "secondary": ["binary", "coding", "debugging", "computer", "programming", "hardware", "cpu", "memory unit", "logic gate", "processor", "compiler", "operating system", "network protocol", "technology", "system", "computation", "digital logic"]},
             2: {"primary": ["emotion", "feeling", "affect", "mood", "sentiment", "psychology", "cognition", "perception", "bias", "stress", "trauma", "joy", "sadness", "anger", "fear", "surprise", "disgust", "empathy"],
                 "secondary": ["myth", "symbolism", "archetype", "metaphor", "narrative structure", "dream analysis", "subconscious", "consciousness studies (psychological)", "attachment theory", "behavioral psychology", "cognitive dissonance"]},
             3: {"primary": ["history", "event", "timeline", "discovery", "science", "physics", "biology", "chemistry", "geology", "astronomy", "year", "date", "century", "civilization", "empire", "war", "revolution", "mineral", "element", "energy", "matter", "force", "motion", "genetics", "evolution"],
                 "secondary": ["archaeology", "anthropology", "society", "invention", "exploration", "culture", "human migration", "industrial revolution", "world war", "cold war", "space race", "internet history"]},
             4: {"primary": ["philosophy", "metaphysics", "ontology", "epistemology", "ethics", "religion", "spirituality", "quantum mechanics", "quantum field theory", "general relativity", "string theory", "consciousness (philosophical/speculative)", "theorem", "paradox", "reality", "existence", "multiverse", "simulation theory", "artificial general intelligence", "emergence", "complexity theory", "chaos theory"],
                 "secondary": ["logic (philosophical)", "reason", "truth", "meaning", "purpose", "free will", "determinism", "theology", "cosmology (speculative)", "future studies", "transhumanism"]}
        }
    ...
    def get_processing_directives(self, phase): # Step 1 changes integrated here
        ...
        # Phase 1 enforcement REMOVED (March 28, 2026):
        # strict_phase1_logic_focus, allow_new_symbol_generation=False, symbolic deferral,
        # and stricter thresholds have been removed. Guidelines tell Sofia where she CAN start.
        # Code never tells her where she CANNOT go. Both brains should be able to receive
        # content from any phase. See SOPHIA_TRUTH_FRAMEWORK.md Correction 5.
        return directives
...
class DynamicBridge: # Incorporating Step 1, 2, 4.1, 7.1 changes with Unified Weight System
    ...
    def _extract_values_from_memory_item(self, item: dict, detected_emotions: dict) -> list:
        """
        Bridge the architectural gap between tripartite memory items and value formation.
        Transforms memory item format into experience-like format for value extraction.

        Args:
            item: Tripartite memory item dict with text, emotions, metadata
            detected_emotions: Emotion detection output from emotion_handler

        Returns:
            List of extracted ValueStatement objects (may be empty)
        """
        if not self.value_formation:
            return []

        try:
            # Calculate emotional intensity from verified emotions
            # emotions dict format: {"joy": 0.85, "curiosity": 0.72, ...}
            emotions = item.get('emotions', {})

            if not emotions:
                return []

            # Calculate max emotion intensity (the strongest feeling)
            max_emotion_score = max(emotions.values()) if emotions else 0.0

            # Calculate average emotion intensity (overall emotional significance)
            avg_emotion_score = sum(emotions.values()) / len(emotions) if emotions else 0.0

            # Transform tripartite item into experience-like format
            # This mimics the structure expected by value_formation.py
            experience_proxy = type('obj', (object,), {
                ...
                'outcome_assessment': {
                    # Map emotion intensity to quality score
                    'quality_score': min(1.0, avg_emotion_score * 1.2),  # Boost slightly

                    # Map max emotion to personal significance
                    'personal_significance_score': max_emotion_score,

                    # Extract insights from keywords if available
                    'insights_gained': item.get('keywords', []),

                    # Include emotional context
                    'personal_significance': f"Emotions detected: {', '.join([f'{k}({v:.2f})' for k,v in emotions.items()])}"
                },
                'interaction_data': {
                    'decision_type': item.get('decision_type', 'unknown'),
                    'logic_score': item.get('logic_score', 0.0),
                    'symbolic_score': item.get('symbolic_score', 0.0),
                    'symbols_found': item.get('symbols_list', []),
                    'processing_phase': item.get('processing_phase', 1),
                    'is_highly_relevant': item.get('is_highly_relevant', False)
                }
            })()

            # Identify value indicators (this duplicates the logic from value_formation.py)
            ...
            print(f"  ⚠️ Value extraction failed: {e}")
            return []
    ...
    def _score_text_for_phase(text_content, phase_directives):
        ...
        # Anti-keyword penalty REMOVED (March 28, 2026). Code should never tell
        # Sofia where she cannot go. See SOPHIA_TRUTH_FRAMEWORK.md Correction 5.
    ...
    def determine_target_storage_phase(self, text_chunk, current_processing_phase_num): # From Step 1 & 2
        ...
        # Phase 1 Symbolic Deferral REMOVED (March 28, 2026):
        # Symbolic content should not be deferred. Both brains need material from any phase.
        # See SOPHIA_TRUTH_FRAMEWORK.md Correction 5.
    ...
    def route_chunk_for_processing(self, text_input, source_url,
        ...
        # VALUE FORMATION: Extract values from emotionally significant experiences
        # Filter: Only extract values if emotional intensity > 0.6 (user directive)
        emotions_dict = item.get('emotions', {})
        max_emotion_intensity = max(emotions_dict.values()) if emotions_dict else 0.0

        if max_emotion_intensity > 0.6:
            extracted_values = self._extract_values_from_memory_item(item, detected_emotions_output)

            if extracted_values:
                # Tag the memory item to create audit trail
                item['value_extracted'] = True
                item['values_extracted_count'] = len(extracted_values)
                item['value_extraction_timestamp'] = datetime.utcnow().isoformat()

                # Update the stored item with the tag
                # Re-store to update metadata (tripartite memory handles duplicates)
                self.tripartite_memory.store(item, decision_type, self.weights)

                print(f"  [ValueFormation] ✨ Extracted {len(extracted_values)} values from emotionally significant experience (emotion intensity: {max_emotion_intensity:.2f})")

                # Log the extracted values
                for val in extracted_values:
                    print(f"    → {val.category}: strength {val.strength:.2f}")
