    ...
    if routing == 'FOLLOW_LOGIC':
            ...
            if recent_items:
                ...
                if hybrid_learnings:
                    ...
                    for item in hybrid_learnings[:2]:
                        response_parts.append(f"   • {item['text'][:600]}..." + (f" [{item['confidence']:.0%}]" if item['confidence'] > 0 else ""))
                ...
                if logic_texts:
                    ...
                    for item in logic_texts[:2]:
                        response_parts.append(f"   • {item['text'][:500]}... (relevance: {item['similarity']:.0%})")
                    response_parts.append("")
                
                if symbolic_results:
                    response_parts.append("🎭 **Symbolic echoes:**")
                    for score, item in symbolic_results[:2]:
                        response_parts.append(f"   • {item.get('text', '')[:500]}... (resonance: {score:.0%})")
                    response_parts.append("")
        ...
        if any(kw in user_input.lower() for kw in knowledge_keywords):
            ...
            if logic_texts:
                ...
                for item in logic_texts[:2]:
                    response_parts.append(f"• {item['text'][:600]}... (relevance: {item['similarity']:.0%})")
            
            if symbolic_results:
                response_parts.append("\n🎭 From symbolic patterns:")
                for score, item in symbolic_results[:2]:
                    response_parts.append(f"• {item.get('text', '')[:600]}... (resonance: {score:.0%})")
...
def show_memory_distribution():
    ...
    print(f"   Bridge (unresolved): {summary['routing_distribution']['hybrid']}")
