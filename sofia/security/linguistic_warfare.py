...
class LinguisticWarfareDetector:
    ...
    def __init__(self, data_dir="data"):
        ...
        try:
            from sofia.utils.learning_config import LearningConfig
            multiplier = LearningConfig.get_threshold_multiplier()
        except:
            multiplier = 1.0
        ...
        self.thresholds = {
            ...
            'manipulation_score': min(0.95, 0.7 * multiplier),  # Base: 0.7, cap at 0.95
        }
    ...
    def _assess_domain_risk(self, domain: str) -> float:
        ...
        try:
            ...
            print(f"[REGEX_ERROR] IP pattern matching failed: {e}")
            # Fallback: check if domain contains only digits and dots
        ...
        try:
            ...
            print(f"[REGEX_ERROR] Subdomain pattern matching failed: {e}")
            # Fallback: check for very long alphanumeric sequences
            parts = domain.split('.')
            for part in parts:
