...
try:
    ...
    hartmann_model = hartmann_model.to(device)  # Move to GPU/CPU
    ...
    distil_model = distil_model.to(device)  # Move to GPU/CPU
    ...
    bert_model = bert_model.to(device)  # Move to GPU/CPU
    ...
    print(f"✅ Emotion models loaded successfully on {device} for emotion_handler.py.")
