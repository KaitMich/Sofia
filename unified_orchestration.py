...
class UnifiedOrchestrationSystem:
    ...
    def __init__(self, data_dir: str = "data", verbose: bool = False):
        ...
        try:
            ...
            print(f"⚠️  Shutdown manager not available: {e}")
        ...
        try:
            ...
            print(f"⚠️  Dream cycle not available: {e}")
        ...
        self.idle_threshold_seconds = 30 * 60  # 30 minutes

        # Register shutdown cleanup handlers if shutdown manager available
    ...
    def _register_shutdown_handlers(self):
        ...
        try:
            ...
            try:
                ...
                if self.verbose:
                    print(f"  [Shutdown] unified_memory cleanup not available: {e}")

            # Register final log cleanup
            log_cleanup = create_log_cleanup("Autonomous system shutdown: Graceful exit")
            self.shutdown_manager.register_cleanup(
                log_cleanup,
            ...
            if self.verbose:
                print("  [Shutdown] Cleanup handlers registered")

        except Exception as e:
            print(f"  ⚠️ Warning: Failed to register some cleanup handlers: {e}")

    async def run_autonomous_loop(self):
        """
        Main autonomous lifecycle loop with idle detection and sleep cycles.

        This implements the 'instinct phase' - continuous operation with:
        ...
        """
        print("\n" + "="*70)
        print("🌟 AUTONOMOUS LIFECYCLE LOOP STARTED")
        print("="*70)
        print(f"Idle threshold: {self.idle_threshold_seconds / 60:.0f} minutes")
        ...
        try:
            while self.autonomous_loop_running:
                # Non-blocking sleep with periodic checks
                await asyncio.sleep(check_interval)

                # Check for idle and trigger sleep if needed
                await self._check_idle_and_sleep()

                # Optional: Add other periodic maintenance tasks here
                # Example: self._check_system_health()

        except KeyboardInterrupt:
            print("\n\n⚠️  Keyboard interrupt detected in autonomous loop")
            ...
            print("\n🔴 Autonomous lifecycle loop stopped")
            self.autonomous_loop_running = False

    async def _check_idle_and_sleep(self):
        """
        Check if system has been idle and trigger sleep cycle if threshold exceeded.
        """
        current_time = time.time()
        idle_duration = current_time - self.last_user_interaction
        ...
        if idle_duration >= self.idle_threshold_seconds:
            ...
            if not self.dream_cycle:
                if self.verbose:
                    print(f"\n⏰ System idle for {idle_duration / 60:.1f} minutes but dream cycle unavailable")
                # Reset timer anyway to avoid spam
                self.last_user_interaction = time.time()
                return

            print(f"\n💤 System idle for {idle_duration / 60:.1f} minutes - triggering sleep cycle...")

            try:
                # Run sleep cycle
                results = self.dream_cycle.run_sleep_cycle()

                # Log results
                print(f"   ✅ Sleep cycle complete:")
                print(f"      NREM: {results['nrem']['items_reclassified']} items consolidated")
                print(f"      REM: {results['rem']['insights_generated']} insights generated")
                print(f"      Duration: {results['duration_seconds']:.1f}s")

                # Reset idle timer after sleep
                self.last_user_interaction = time.time()

            except Exception as e:
                print(f"   ❌ Sleep cycle failed: {e}")
                # Don't reset timer if sleep failed - will retry next check

        else:
            # Log idle status periodically (every 10 minutes of idle time)
            if idle_duration > 0 and int(idle_duration) % 600 == 0:
                remaining = (self.idle_threshold_seconds - idle_duration) / 60
                if self.verbose:
                    print(f"   ⏰ Idle: {idle_duration / 60:.1f} min (sleep in {remaining:.0f} min)")
    ...
    def update_interaction_time(self):
        ...
        self.last_user_interaction = time.time()
        if self.verbose:
            print(f"   🔄 Interaction timestamp updated")
